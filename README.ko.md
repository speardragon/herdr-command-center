...
macOS에서는 배려가 하나 더 있습니다. 한국어 IME처럼 ASCII가 아닌 입력 소스가 켜진
채로 팝업이 열리면 마지막에 쓰던 ASCII 자판으로 잠깐 전환합니다 — 키 하나로
움직이는 TUI는 모든 키가 조합 중인 글자로 빨려 들어가면 무용지물이니까요. 팝업이
닫히는 순간 — Esc든, 커맨드 실행이든, 페인이 죽든 — 분리된 감시 프로세스가 원래
입력 소스를 되돌립니다. `COMMAND_CENTER_ASCII_INPUT=0`으로 끌 수 있습니다.
